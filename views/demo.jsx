...
//import getCustId from './get-custid'; // TODO
...
export class Demo extends Component {
  ...
  getRecognizeOptions(extra) {
    ...
    // 取得したCustomization IDの形式(GUID)をチェックして、有効かどうかをチェックする
    const languageCustomizationId = document.getElementById("custid").value;
    const pattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
    if (pattern.test(languageCustomizationId) === true) {
      console.log(`CUST_ID: ${languageCustomizationId}`);
      Options["languageCustomizationId"] = languageCustomizationId;
    } else {
  ...
  handleModelChange(model) {
    ...
    // TODO: customization_idを取得して、セットする
    //let Options = {
    //  accessToken: this.state.accessToken,
    //  token: this.state.token,
    //}
    //const myCustid = getCustId(Options);
    //console.log(myCustid);

    // customization_idのリセット
    const languageCustomizationId = document.getElementById("custid");
    languageCustomizationId.value = "";
      ...
      >
        ...
        <div className="flex setup">
          <div className="column">
            ...
            <p>Customization ID: <input
                type="text"
                id="custid"
                placeholder="Type your customization_id here (optional)"
